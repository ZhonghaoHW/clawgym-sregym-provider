...
def _handoff(run="a" * 64, release="b" * 64):
    value = {
        ...
        "verification_plan": ["reread"],
    }
    value["handoff_digest"] = hashlib.sha256(
...
def test_parse_rejects_compound_command():
    assert (
        parse_command("kubectl delete networkpolicy deny-all-recommendation -n hotel-reservation; kubectl get pods")[0]
        == "unknown"
    )
...
def test_gate_requires_exact_transaction():
    ...
    assert gate.record(
        "kubectl get networkpolicy deny-all-recommendation -n hotel-reservation", "policy exists", stage="mitigation"
    )
    assert gate.record(
        "kubectl delete networkpolicy deny-all-recommendation -n hotel-reservation", "deleted", stage="mitigation"
    )
    ...
    assert gate.record(
        "kubectl get endpoints recommendation -n hotel-reservation", "addresses: 10.0.0.1", stage="mitigation"
    )
