...
def test_backend_requires_exact_target_and_emits_expected_state_sequence() -> None:
    ...
    backend = SREGymEnvironmentQualificationBackend(
        ...
        provision=lambda trial: calls.append("provision"),
        inject=lambda trial: calls.append("inject"),
        observe=observe,
        recover=lambda trial: calls.append("recover"),
        cleanup=lambda trial: calls.append("cleanup"),
        tool_probe=lambda: {"passed": True},
        isolation_probe=lambda: {"passed": True},
        profile_digest="a" * 64,
...
def test_backend_receipts_redact_provider_payloads() -> None:
    ...
    backend = SREGymEnvironmentQualificationBackend(
        ...
        inject=lambda _trial: None,
        observe=observe,
        recover=lambda _trial: None,
        cleanup=lambda _trial: None,
        tool_probe=lambda: {"passed": True},
        isolation_probe=lambda: {"passed": True},
        profile_digest="b" * 64,
        release_role="candidate",
    ...
    assert all(
        set(receipt["result"]) <= {"target_path", "non_target_healthy", "oracle", "cleanup", "summary_digest"}
