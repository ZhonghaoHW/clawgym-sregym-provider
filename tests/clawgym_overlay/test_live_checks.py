...
def test_kubernetes_telemetry_uses_fixed_encoded_raw_proxy_queries() -> None:
    calls = []

    class Kubectl:
        def exec_command(self, command):
            calls.append(command)
            if "api/traces" in command:
                return '{"data":[{"traceID":"not-exported"}]}'
            return '{"data":{"result":[{"value":"not-exported"}]}}'

    conductor = SimpleNamespace(kubectl=Kubectl())
    ...
    assert len(calls) == 3
    assert all(call.startswith("kubectl get --raw ") for call in calls)
    assert "query=up" in calls[0]
    assert "query_range" in calls[1]
    assert "%7Bnamespace%3D%22hotel-reservation%22%7D" in calls[1]
    assert "service=frontend&limit=20" in calls[2]
