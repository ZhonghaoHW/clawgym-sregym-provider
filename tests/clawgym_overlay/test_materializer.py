...
def _inputs(tmp_path):
    parent = {
        "schema_id": "clawgym.sregym_reference_agent_profile.v1",
        ...
        "agent_release_digest": F,
        "tool_policy_profile_bundle_digest": D,
    }
    bundle = {
        "schema_id": "agent_evolution.reference_agent_component_bundle.v1",
        "bundle_id": "b",
        "experimental_baseline_digest": D,
        "base_agent_release_digest": F,
        "search_plan_digest": G,
        "train_partition_digest": D,
        "components": {
            "diagnosis": {"system": "Diagnose {app_name}", "user": "Inspect {app_namespace}", "summary": "Summarize"},
            "mitigation": {
                "system": "Fix {app_name}",
    ...
    proposal = {
        ...
        "candidate_id": "c",
        "experimental_baseline_digest": D,
        "frozen_control_agent_release_digest": E,
        "base_agent_release_digest": F,
        "environment_release_digest": G,
        "search_plan_digest": D,
        "component_bundle_digest": bundle["component_bundle_digest"],
        "source_train_episode_digests": [D],
        "parent_lineage": [E, F],
...
def test_materialization_is_deterministic_and_explicit(tmp_path):
    ...
    r1 = materialize_reference_profile(
        proposal_path=p,
        component_bundle_path=b,
        parent_profile_path=parent,
        output_dir=tmp_path / "one",
        runtime_reference="a" * 40,
    )
    r2 = materialize_reference_profile(
        proposal_path=p,
        component_bundle_path=b,
        parent_profile_path=parent,
    ...
    assert load_materialized_reference_profile(tmp_path / "one", profile_digest=r1["profile_digest"])[
        "schema_id"
    ].endswith("v2")
...
def test_materializer_rejects_tamper_and_reuse(tmp_path):
    ...
    doc["components"]["diagnosis"]["system"] = "bad {unknown}"
    _write(b, doc)
