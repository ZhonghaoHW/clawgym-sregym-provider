...
# Public alias for live qualification composition roots; historical internal
# users continue to resolve the private constant without semantic changes.
TARGET = _TARGET
_FAMILIES = {
...
def _safe_observation(value: Any) -> dict[str, Any]:
    ...
    """
    raw: Mapping[str, Any] = cast(Mapping[str, Any], value) if isinstance(value, Mapping) else {}
    safe: dict[str, Any] = {}
    ...
    if isinstance(summary, Mapping):
        safe["summary_digest"] = sha256_digest(dict(cast(Mapping[str, Any], summary)))
    return safe
...
class EnvironmentControlProfile:
    ...
    def from_dict(cls, document: Mapping[str, Any]) -> EnvironmentControlProfile:
        ...
        normalized: dict[str, tuple[str, ...]] = {
            str(key): tuple(str(item) for item in cast(list[Any] | tuple[Any, ...], value))
            for key, value in cast(Mapping[str, Any], families).items()
            if isinstance(value, (list, tuple))
        }
        profile = cls(
            document["profile_id"], revision, normalized, document["target"], tuple(document["required_states"])
        )
...
def default_environment_control_profile(revision: str) -> dict[str, Any]:
    ...
    profile = EnvironmentControlProfile(
        "sregym-environment-control-v1",
        revision,
        {key: tuple(sorted(values)) for key, values in _FAMILIES.items()},
        _TARGET,
    )
...
class SREGymEnvironmentQualificationBackend:
    ...
    def run(self, *, trial_id: str, seed: int, attempt_id: str) -> dict[str, Any]:
        if not trial_id or not attempt_id or isinstance(seed, bool):
            raise ValueError("trial identity is invalid")
        receipts: list[dict[str, Any]] = []
        self.reset()
        baseline = dict(self.observe("baseline"))
        receipts.append({"phase": "baseline", "result": _safe_observation(baseline)})
        self.provision(trial_id)
        self.inject(trial_id)
        injected = dict(self.observe("injected"))
        receipts.append({"phase": "injected", "result": _safe_observation(injected)})
        tool = dict(self.tool_probe())
        isolation = dict(self.isolation_probe())
        self.recover(trial_id)
        recovered = dict(self.observe("recovered"))
        receipts.append({"phase": "recovered", "result": _safe_observation(recovered)})
        self.cleanup(trial_id)
        cleaned = dict(self.observe("cleaned"))
        receipts.append({"phase": "cleaned", "result": _safe_observation(cleaned)})
        expected = {"baseline": "pass", "injected": "fail", "recovered": "pass", "cleaned": "pass"}
        state_oracle = {
            name: item.get("oracle")
            for name, item in (
                ("baseline", baseline),
                ("injected", injected),
                ("recovered", recovered),
        ...
        return {
            ...
            "target": dict(cast(Mapping[str, str], self.target)),
            "state_oracle": state_oracle,
            "target_path": {
                name: item.get("target_path")
                for name, item in (
                    ("baseline", baseline),
                    ("injected", injected),
                    ("recovered", recovered),
            ...
            "non_target_healthy": all(
                item.get("non_target_healthy") is True for item in (baseline, injected, recovered, cleaned)
            ),
            "tool_usable": tool.get("passed") is True,
            "isolated": isolation.get("passed") is True,
            "cleanup": cleaned.get("cleanup") is True,
            "status": "completed" if passed else "semantic_disqualified",
            "failure_class": None if passed else "semantic_disqualified",
            "receipts": receipts,
        }
