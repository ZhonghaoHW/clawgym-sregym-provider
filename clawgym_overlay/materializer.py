...
_SECRET_LIKE = re.compile(
    r"-----BEGIN|(?:api[_-]?key|secret)\s*[:=]|\bsk-[A-Za-z0-9]{8,}|(?:^|\s)/(?:[A-Za-z0-9._-]+/)+", re.IGNORECASE
)
_ALLOWED = {
...
def materialize_reference_profile(
    *,
    ...
    bundle = _read(Path(component_bundle_path))
    parent = _read(Path(parent_profile_path))
    ...
    if proposal.get("component_bundle_digest") != bundle["component_bundle_digest"] or proposal.get(
        "base_agent_release_digest"
    ) != parent.get("agent_release_digest", proposal.get("base_agent_release_digest")):
        raise MaterializationError("proposal/component/parent identity mismatch")
    if parent.get("schema_id") not in {
        "clawgym.sregym_reference_agent_profile.v1",
        "clawgym.sregym_reference_agent_profile.v2",
    }:
    ...
    if set(parent) - {
        ...
        "tool_policy_profile_bundle_digest",
        "profile_digest",
        "runtime_protocol",
    ...
    if not all(
        isinstance(bundle.get(k), int) and not isinstance(bundle.get(k), bool) and 1 <= bundle[k] <= 8
        for k in ("diagnosis_step_limit", "mitigation_step_limit")
    ):
    ...
    components = bundle["components"]
    rendered = {
        "reference-materialized/diagnosis_agent_config.yaml": yaml.safe_dump(
            {
                "max_step": bundle["diagnosis_step_limit"],
                "sync_tools": None,
                "async_tools": [
                    {"name": "get_traces"},
                    {"name": "get_services"},
                ...
                "handoff_protocol": "clawgym.sregym_diagnosis_handoff.v2",
                "handoff_argument_protocol": "structured-submit-tool-argument-v1",
            },
            sort_keys=True,
            allow_unicode=True,
        ),
        "reference-materialized/mitigation_agent_config.yaml": yaml.safe_dump(
            {
                "max_step": bundle["mitigation_step_limit"],
                "max_retry_attempts": 1,
                "retry_mode": "validate",
                "sync_tools": [{"name": "wait_tool"}],
                "async_tools": [
                    {"name": "get_traces"},
                    {"name": "get_services"},
                    {"name": "get_operations"},
                ...
                "transaction_protocol": "clawgym.sregym_remediation_transaction.v2",
            },
            sort_keys=True,
            allow_unicode=True,
        ),
        "reference-materialized/diagnosis_agent_prompts.yaml": yaml.safe_dump(
            {
                "system": components["diagnosis"]["system"],
                "user": components["diagnosis"]["user"],
                "diagnosis_summary_prompt": components["diagnosis"]["summary"],
            },
            sort_keys=True,
            allow_unicode=True,
        ),
        "reference-materialized/mitigation_agent_prompts.yaml": yaml.safe_dump(
            {
                "system": components["mitigation"]["system"],
                "user": components["mitigation"]["user"],
                "retry_user": components["mitigation"]["retry_user"],
            },
            sort_keys=True,
            allow_unicode=True,
        ),
    ...
    file_summary = [
        {
            "path": path,
            "container_path": "/opt/sregym/clients/stratus/configs/" + Path(path).name,
            "sha256_digest": hashlib.sha256(content.encode()).hexdigest(),
    ...
    profile.update(
        {
            ...
            "handoff_argument_protocol": "structured-submit-tool-argument-v1",
            "bounded_execution": {
                "diagnosis_max_steps": bundle["diagnosis_step_limit"],
                "mitigation_max_steps": bundle["mitigation_step_limit"],
                "container_timeout_seconds": 900,
            },
            "config_bundle_digest": config["config_bundle_digest"],
            "component_bundle_digest": bundle["component_bundle_digest"],
            "semantic_component_digest": component_digest,
            "materializer_runtime_reference": runtime_reference,
        }
    )
    profile["profile_digest"] = sha256_digest(profile)
    receipt = {
        "schema_id": "clawgym.agent_materialization_receipt.v1",
        "proposal_digest": proposal["proposal_digest"],
        "component_bundle_digest": bundle["component_bundle_digest"],
        "parent_agent_release_digest": proposal["base_agent_release_digest"],
        "environment_release_digest": proposal["environment_release_digest"],
        "adapter_id": profile["adapter_id"],
        "materializer_id": "clawgym.reference-profile-materializer",
        "materializer_revision": runtime_reference,
        "profile_digest": profile["profile_digest"],
        "config_bundle_digest": config["config_bundle_digest"],
        "semantic_component_digest": component_digest,
        "tool_policy_profile_bundle_digest": parent.get("tool_policy_profile_bundle_digest", ""),
...
def main(argv: list[str] | None = None) -> int:
    ...
    receipt = materialize_reference_profile(
        proposal_path=args.proposal,
        component_bundle_path=args.component_bundle,
        parent_profile_path=args.parent_profile,
